...
function isDynamic(path, { checkMember, checkTags, checkCallExpressions = true, native }) {
  ...
  if (
    ...
    (checkMember && t__namespace.isMemberExpression(expr)) ||
...
function transformCondition(path, deep) {
  ...
  if (
    ...
    if (expr.operator === "||" && t__namespace.isLogicalExpression(expr.left)) {
    ...
    if (dTest) {
      ...
      id = path.scope.generateUidIdentifier("_c$");
      if (expr.operator !== "||" && !t__namespace.isBinaryExpression(cond))
        cond = t__namespace.unaryExpression("!", t__namespace.unaryExpression("!", cond, true), true);
...
function registerTemplate(path, results) {
  ...
  if (results.template.length) {
    ...
    decl = t__namespace.variableDeclarator(
      ...
      hydratable
        ? t__namespace.callExpression(
            t__namespace.identifier("_$getNextElement"),
            [templateId]
          )
...
function transformElement(path, info) {
  ...
  if (info.topLevel && config.hydratable) {
    ...
    results.templateValues.push(t__namespace.callExpression(t__namespace.identifier("_$getHydrationKey"), []));
  }
